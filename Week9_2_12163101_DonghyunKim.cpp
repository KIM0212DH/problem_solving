...
int CandidateCheckFunc(set<point_info> candidate,vector<vector<vector<int>>>check) {
	int ret = 0;

	for (point_info point = candidate.begin; point < candidate.size(); i++) {
		check[point.x][point.y].clear();
	}

}

void aroundCandidateFunc(int x, int y, int**grid) {
	for (int i{ 1 }; i < 9; i++) {
		int X = x + dx[i];
		int Y = y + dy[i];
		if (grid[X][Y] == 0) {
...
int main() {
	...
	while (t--) {
		...
		for (int i{ 0 }; i < 2; i++) {
			for (int j{ 0 }; j < 2; j++) {
				aroundCandidateFunc(n / 2 + i, n / 2 + j, grid);
			}
		}
		...
		while (q--) {
			...
			if (check[x][y].empty() == true) {
				continue;
			}
			for (int k{ 1 }; k < check[x][y].size()+1; k++) {
				int i = check[x][y][k];
				for (int j{ 1 }; j < n + 1; j++) {
